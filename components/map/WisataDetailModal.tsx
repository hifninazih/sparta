...
import { 
  Star, MapPin, Navigation, Phone, ExternalLink, AtSign, Info, Sparkles, Quote 
} from "lucide-react";
...
export function WisataDetailModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto p-0 sm:max-w-2xl border-2 border-black rounded-xl gap-0">
        ...
        <div className="relative flex items-start justify-between border-b-2 border-black bg-slate-100 p-6 pb-4">
          <div className="flex flex-col gap-3 pr-8">
            ...
            <div>
              <h2 className="text-2xl leading-tight font-black text-black mb-1">
                {wisata.name}
              </h2>
              
              <div className="flex flex-wrap items-center gap-3 text-sm font-bold text-slate-700 mt-2">
                <span className="flex items-center gap-1 text-amber-500 bg-amber-50 px-2 py-1 border-2 border-amber-200 rounded-md">
                ...
                <span className="text-sm font-black text-green-700 bg-green-50 px-2 py-1 border-2 border-green-200 rounded-md">
                  {wisata.price === 0
                    ? "GRATIS"
                    : wisata.price
        ...
        <div className="flex flex-col p-6 space-y-6 bg-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-3 text-sm font-bold text-slate-700">
              <h4 className="text-xs uppercase tracking-wider text-slate-400 mb-1 border-b-2 border-slate-100 pb-1">Lokasi</h4>
              <div className="flex items-start gap-2">
                ...
                <span className="leading-snug capitalize">
                  {[
                    ...
                    wisata.kabupaten,
                  ].filter(Boolean).join(", ") || wisata.address || "Tidak ada informasi alamat"}
                </span>
              </div>
              <Button
            ...
            <div className="flex flex-col gap-3 text-sm font-bold text-slate-700">
              <h4 className="text-xs uppercase tracking-wider text-slate-400 mb-1 border-b-2 border-slate-100 pb-1">Kontak & Media</h4>
              ...
              {wisata.phone ? (
                ...
                <div className="flex items-center gap-2 text-slate-400 opacity-70">
                  ...
                  <span className="font-medium text-xs italic">Tidak ada telepon</span>
                </div>
              )}
              ...
              {wisata.link ? (
                <div className="flex items-center gap-2">
                  ...
                  <a href={wisata.link} target="_blank" rel="noreferrer" className="text-purple-600 hover:underline break-all">
                    Website Resmi
                  </a>
                </div>
              ) : (
                <div className="flex items-center gap-2 text-slate-400 opacity-70">
                  ...
                  <span className="font-medium text-xs italic">Tidak ada website</span>
                </div>
              )}
              ...
              {wisata.username_instagram ? (
                <div className="flex items-center gap-2">
                  ...
                  <a href={`https://instagram.com/${wisata.username_instagram.replace("@", "")}`} target="_blank" rel="noreferrer" className="text-pink-600 hover:underline">
                    {wisata.username_instagram.startsWith("@") ? wisata.username_instagram : `@${wisata.username_instagram}`}
                  </a>
                </div>
              ) : (
                <div className="flex items-center gap-2 text-slate-400 opacity-70">
                  <AtSign className="h-4 w-4 shrink-0" />
                  <span className="font-medium text-xs italic">Tidak ada instagram</span>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Section: Daya Tarik */}
          {(wisata.daya_tarik_utama || wisata.daya_tarik_pendukung || wisata.sub_kategori) && (
            <div className="flex flex-col gap-4 border-2 border-black rounded-lg p-4 bg-yellow-50 shadow-[4px_4px_0px_rgba(0,0,0,1)]">
              {wisata.sub_kategori && (
